...
<!--                        <img  src="images/create-your-own/Women.png" title="WOMEN" alt="image 1" id="image1" class="image-toggle img-responsive" />-->
            ...
            <fieldset id="tab6">
                ...
                <div class="col-sm-12 col-xs-12 step_col_left">
                    <input class="max-len-limit1" type="text" name="line-1" id="line-1" value="" placeholder="Enter label1 text here" maxlength=""/>
                    <p class="max-len1" style="float:right;font-size: 11px;"></p>
                    <input class="max-len-limit2" type="text" name="line-2" id="line-2" value="" placeholder="Enter label2 text here" maxlength=""/>
                    <p class="max-len2" style="float:right;font-size: 11px;"></p>
                    <form action="" method="post" enctype="multipart/form-data" id="image_form" name="image_form">
                ...
                <div class="col-sm-12 col-xs-12 step_col_right">
                    <div id="label_iamge" style="position:relative">
                        <div id="image" class="data_style">
                            <img src="" class="img-responsive" id="bottle_custom_image" alt=""/>
                        </div>
                        <div id="label_1" class="data_style"></div>
                        <div id="label_2" class="data_style"></div>
                    </div>
            ...
            <fieldset id="tab7">
                ...
                <div class="col-sm-12 col-xs-12 step_col_right">
                    <div id="label_iamge_tab7" style="position:relative">
                        <div id="image_tab7" class="data_style">
                            <img src="" class="img-responsive" id="bottle_custom_image_tab7"/>
                        </div>
                        <div id="label_1_tab7" class="data_style"></div>
                        <div id="label_2_tab7" class="data_style"></div>
                    </div>



                </div>
                <!--				<div class="col-sm-12 col-xs-12 step_col_left" style="width: 435px;">
                                                        <p style="text-align: center;font-weight: 600;padding-bottom: 15px;" id="done-heading"></p>
                                                        <div class="row">
                                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                                        <h3 style="text-align:left;font-size: 20px;font-weight: 600;" id="first_line"></h3>
...
<script>
    $(document).ready(function () {
        ...
        $("#image_form").submit(function (e) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= Yii::$app->homeUrl; ?>ajax/bottle-image-save', // Upload Script
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function (data) {
                    ...
                    $('#bottle_custom_image').attr('src', data);
        ...
        $("#line-1").keyup(function () {
            var label_1 = $('#line-1').val();
            $('#label_1').text(label_1);
        });
        $("#line-2").keyup(function () {
            var label_2 = $('#line-2').val();
        ...
        $(document).on('change', 'input[type=radio][name=bottle]', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#image').css("height", positions['image']['height']);
                    var top_value = parseInt(positions['image']['top']);
                    var left_value = parseInt(positions['image']['left']);
                    $('#bottle_id').val(res.result['id']);

                    $("#image").css({"width": positions['image']['width'], "height": positions['image']['height'], "top": top_value + 'px', "left": left_value + 'px', 'position': 'absolute'});
                    $("#label_1").css({"width": positions['label_1']['width'], "top": parseInt(positions['label_1']['top']) + 'px', "left": parseInt(positions['label_1']['left']) + 'px', position: 'absolute'});
                    $("#label_2").css({"width": positions['label_2']['width'], "font_size": positions['label_2']['font_size'] + 'px', "top": parseInt(positions['label_2']['top']) + 'px', "left": parseInt(positions['label_2']['left']) + 'px', position: 'absolute'});
                    $("#label_1").css("font-size", positions['label_1']['font_size'] + 'px');
                    $("#label_2").css("font-size", positions['label_2']['font_size'] + 'px');
                    $('#label_iamge').css("background-image", "url(" + res.result['bottle-src'] + ")");
                    $('#label_iamge').css({"width": res.result['img_width'] + 'px', 'height': res.result['img_height'] + 'px'});
                    $('#bottle1_image').attr('src', res.result['bottle-src']);
                    $('.max-len1').text(res.result['max-length1']);
                    $('.max-len2').text(res.result['max-length2']);
                    $('.max-len-limit1').attr('maxlength', res.result['max-limit1']);
                    $('.max-len-limit2').attr('maxlength', res.result['max-limit2']);
                    $("#bottle_custom_image").show();
                    $("#bottle_custom_image").css({"width": positions['image']['width'], "height": positions['image']['height']});
...
<script src="<?= yii::$app->homeUrl; ?>js/happy-slider.js"></script>
