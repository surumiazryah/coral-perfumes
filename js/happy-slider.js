(function (i, g, c, h, e, k, f) { /*! Jssor */
	...
	var b = i.$Jssor$ = new function () {
		...
		function Gb() {
			if (!L) {
				...
				if (E.pointerEnabled || (a = E.msPointerEnabled))
					L.re = a ? "msTouchAction" : "touchAction"
			}
			return L
		}
		...
		j.lg = function (d, a) {
			if (a)
				d.style.clip = "rect(" + c.round(a.$Top || a.H || 0) + "px " + c.round(a.$Right) + "px " + c.round(a.$Bottom) + "px " + c.round(a.$Left || a.F || 0) + "px)";
			else if (a !== f) {
		...
		j.wd = function (l, m, x, q, z, A, n) {
			...
			if (a.$Clip && b.kg() && !a.$Clip.$Top && !a.$Clip.$Left && !a.$Clip.H && !a.$Clip.F && a.$Clip.$Right == n.$OriginalWidth && a.$Clip.$Bottom == n.$OriginalHeight)
				a.$Clip = h;
			return a
		}
	};
	...
	i.$JssorSlideshowRunner$ = function (n, s, q, u, z) {
				...
				b.$Each(h, function (G) {
					...
					if (a.$Zoom || a.$Rotate) {
						...
						if (H) {
							...
							f.$Zoom = 1;
							if (b.Qb() || b.$IsBrowserOpera())
								d.$Zoom = c.min(d.$Zoom, 2);
							var N = a.$Rotate || 0;
		...
		if (b.Qb() || b.$IsBrowserOpera() || z && b.$WebKitVersion() < 537)
			i = 16;
		m.call(f);
		l.call(f, -1e7, 1e7)
	};
	var j = i.$JssorSlider$ = function (q, jc) {
			...
			function W(a, d, c, h) {
				...
				b.$Each(i, function (f) {
					...
					if (j == "caption") {
						...
						} else if (i == "IMG" && b.$AttributeEx(f, "u") == "image")
							n = f;
						if (n) {
							n.border = 0;
							b.M(n, V)
		...
		if (r) {
			...
			a.$ArrowKeyNavigation && b.$AddEvent(g, "keydown", function (b) {
				if (b.keyCode == 37)
					pb(-a.$ArrowKeyNavigation, e);
				else
					b.keyCode == 39 && pb(a.$ArrowKeyNavigation, e)
			});
			var lb = a.$StartIndex;
			if (!(E & 1))
				lb = c.max(0, c.min(lb, r - v));
			A.bc(lb, lb, 0)
		}
	};
	j.$EVT_CLICK = 21;
